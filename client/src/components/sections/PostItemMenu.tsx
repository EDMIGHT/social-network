...
type PostItemMenuProps = Pick<ResponsePost, 'id' | 'likedBy'>;

const PostItemMenu: React.FC<PostItemMenuProps> = ({ id, likedBy }) => {
  const { user, accessToken } = useAppSelector((state) => state.user);

  const [likePost, { isLoading }] = useLikePostMutation();

  const isExist = user && likedBy.some((userLike) => userLike.id === user.id);
    ...
    <div className='flex items-center justify-between gap-2'>
      <button onClick={onClickLike} disabled={isLoading} className='flex items-center gap-2'>
        <svg
          ...
          strokeWidth='1.5'
          stroke='currentColor'
          className={cn('h-6 w-6', isExist && 'fill-primary')}
        >
          <path
            strokeLinecap='round'
